...
function posta_odeslane($db) {

	global $users_class, $PAGE_SIZE, $POSTA_ODESLANA;
	...
	$query = "
		SELECT p.*, u.login
		FROM posta p LEFT JOIN users_sys u ON  p.id_prijemce = u.id  
		WHERE p.id_autor = '$id_hrac' 	
		ORDER BY p.cas DESC
	...
	while ($row = $db->GetFetchAssoc( $res )){
		...
		$text .= "<a href=\"posta.php?section=$POSTA_ODESLANA&amp;detail=${row['id']}\">";
	...
	if ($page > 0) {
		$text .= "<a href=\"posta.php?section=$POSTA_ODESLANA&amp;page=" . ($page - 1) . "\">novější</a>\n";
	}
	else  {
	...
	if ($count == ($PAGE_SIZE+1)) {
		$text .= "<a href=\"posta.php?section=$POSTA_ODESLANA&amp;page=" . ($page + 1) . "\">starší</a>\n";
	} 
	else {
